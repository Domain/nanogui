...
public:
    ...
    override void draw(NVGContext nvg)
    {
        ...
        {
            nvg.fillColor(Color(mBackgroundColor.rgb, 255));
            nvg.fill;
            if (mPushed)
            {
                gradTop.a = gradBot.a = 0.8f * 255;
            }
            else
            {
                const v = 1 - mBackgroundColor.w / 255f;
                gradTop.a = gradBot.a = mEnabled ? v * 255 : (v * 0.5f + 0.5f) * 255;
            }
        }
