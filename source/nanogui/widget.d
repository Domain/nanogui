...
public:
	...
	*/
	...
	auto add(WidgetClass, Args...)(Args args)
	{
	...
	final Popup contextMenu() { return mMenu; }
	final const(Popup) contextMenu() const { return mMenu; }
	/// Set the associated popup menu
	final void contextMenu(Popup popup) { mMenu = popup; }
	...
	*/
	...
	bool mouseButtonEvent(Vector2i p, MouseButton button, bool down, int modifiers)
	{
		...
		switch (button)
		{
			...
			case MouseButton.Right:
				if (!down && mMenu !is null)
				{
					requestFocus();
					mMenu.anchorPos(Vector2i(p.x + 15, p.y + mMenu.anchorHeight - 30));
					mMenu.visible(true);
					return true;
				}
				break;
	...
	/// Handle IME editing event
	bool keyboardEditingEvent(string text, int start, int length)
	{
		return false;
	}
