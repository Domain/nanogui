...
class Screen : Widget
{
	...
	override void draw(NVGContext nvg)
	{
		...
		if (elapsed > 0.5f)
		{
			...
			if (widget && widget.tooltip.length)
			{
				...
				Vector2i pos = widget.absolutePosition() + Vector2i(widget.width() / 2,
						widget.height() + 10);

				nvg.textBounds(pos.x, pos.y, widget.tooltip, bounds);
				...
				if (h > tooltipWidth / 2)
				{
					...
					algn.top = true;
					nvg.textAlign(algn);
					nvg.textBoxBounds(pos.x, pos.y, tooltipWidth, widget.tooltip, bounds);
				...
				nvg.roundedRect(bounds[0] - 4 - h, bounds[1] - 4,
						cast(int)(bounds[2] - bounds[0]) + 8, cast(int)(bounds[3] - bounds[1]) + 8,
						3);
				...
				nvg.fontBlur(0.0f);
				nvg.textBox(pos.x - h, pos.y, tooltipWidth, widget.tooltip);
			}
		}
		else
			mTooltipShown = false;
	...
	bool mouseButtonCallbackEvent(MouseButton button, MouseAction action,
			int modifiers, long timestamp)
	{
		mNeedToDraw = true;
		...
		try
		{
			...
			if (mDragActive && action == MouseAction.Release && dropWidget !is mDragWidget)
			...
			return mouseButtonEvent(mMousePos, button, action == MouseAction.Press, mModifiers);
		}
		catch (Exception e)
		{
	...
	override bool keyboardEditingEvent(string text, int start, int length)
	{
		if (mFocusPath.length)
		{
			foreach_reverse (w; mFocusPath)
			{
				if (w is this)
					continue;
				if (w.focused && w.keyboardEditingEvent(text, start, length))
					return true;
			}
		}
		return false;
	...
	bool needToDraw() const pure @safe nothrow
	{
		return mNeedToDraw;
	}
	...
	{
		if (mNVG !is null && window.size.x == 0 && window.size.y == 0) {
			mNVG.beginFrame(size.x, size.y); // begin rendering
			scope (exit) mNVG.endFrame(); // and flush render queue on exit
        	window.size = window.preferredSize(mNVG);
        	window.performLayout(mNVG);
    	}
    	window.position = (mSize - window.size) / 2;
	}
