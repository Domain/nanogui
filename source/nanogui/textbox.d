...
import nanogui.common : Vector2i, MouseAction, MouseButton, Cursor, Color,
    boxGradient, fillColor, Vector2f, Key, KeyAction, KeyMod, setClipboardText,
    getClipboardText, hasClipboardText;
...
public:
    ...
    this(Widget parent, string value = "Untitled")
    {
        ...
        contextMenu = new Popup(parentWindow.parent, window);
        contextMenu.size(Vector2i(320, 250));
        contextMenu.visible(false);

        import nanogui.layout : GroupLayout;
		contextMenu.layout = new GroupLayout(2, 2);

        Button copyBtn = new Button(contextMenu, "Copy   Ctrl+C");
        copyBtn.callback = () {
			contextMenu.visible = false;
		};
        Button cutBtn = new Button(contextMenu, "Cut    Ctrl+X");
        cutBtn.callback = () {
			contextMenu.visible = false;
		};
        Button pasteBtn = new Button(contextMenu, "Paste   Ctrl+P");
        pasteBtn.callback = () {
			contextMenu.visible = false;
		};
    }
    ...
    final bool spinnable() const
    {
        return mSpinnable;
    }

    final void spinnable(bool spinnable)
    {
    ...
    final string defaultValue() const
    {
        return mDefaultValue;
    }

    final void defaultValue(string defaultValue)
    {
        mDefaultValue = defaultValue;
    }

    final Alignment alignment() const
    {
        return mAlignment;
    }

    final void alignment(Alignment al)
    {
        mAlignment = al;
    }

    final string units() const
    {
        return mUnits;
    }

    final void units(string units)
    {
        mUnits = units;
    }

    final auto unitsImage() const
    {
        return mUnitsImage;
    }

    final void unitsImage(NVGImage image)
    {
        mUnitsImage = image;
    }

    /// Return the underlying regular expression specifying valid formats
    final string format() const
    {
        return mFormat;
    }
    /// Specify a regular expression specifying valid formats
    final void format(string format)
    {
        mFormat = format;
    }

    /// Return the placeholder text to be displayed while the text box is empty.
    final string placeholder() const
    {
        return mPlaceholder;
    }
    /// Specify a placeholder text to be displayed while the text box is empty.
    final void placeholder(string placeholder)
    {
    ...
    final bool delegate(string str) callback() const
    {
        return mCallback;
    }

    /// Sets the callback to execute when the value of this TextBox has changed.
    final void callback(bool delegate(string str) callback)
    {
        mCallback = callback;
    }
        ...
        if (mEditable && focused)
        {
            if (action == KeyAction.Press || action == KeyAction.Repeat)
            {
                ...
                if (mEditingValue.length == 0)
                {
                    ...
                    else if (key == 'a' && modifiers & KeyMod.Ctrl)
                    {
                        mCursorPos = cast(int) mValueTemp.byGrapheme.walkLength;
                        mSelectionPos = 0;
                    }
                    else if (key == 'x' && modifiers & KeyMod.Ctrl)
                    {
                        copySelection();
                        deleteSelection();
                    }
                    else if (key == 'c' && modifiers & KeyMod.Ctrl)
                    {
                        copySelection();
                    }
                    else if (key == 'v' && modifiers & KeyMod.Ctrl)
                    {
                        deleteSelection();
                        pasteFromClipboard();
                    }
    ...
    override bool keyboardCharacterEvent(dchar codepoint)
    {
        if (mEditable && focused)
        {
            ...
            mValueTemp = mValueTemp[0 .. pos] ~ replacement ~ mValueTemp[pos .. $];
            mCursorPos++;
    ...
    override bool keyboardEditingEvent(string text, int start, int length)
    {
        if (mEditable && focused)
        {
            mEditingValue = text;
            mEditingCursorPos = start;
            return true;
        }
    ...
    override void draw(NVGContext nvg)
    {
        ...
        NVGPaint bg = nvg.boxGradient(mPos.x + 1, mPos.y + 1 + 1.0f, mSize.x - 2,
                mSize.y - 2, 3, 4, Color(255, 255, 255, 32), Color(32, 32, 32, 32));
        NVGPaint fg1 = nvg.boxGradient(mPos.x + 1, mPos.y + 1 + 1.0f, mSize.x - 2,
                mSize.y - 2, 3, 4, Color(150, 150, 150, 32), Color(32, 32, 32, 32));
        NVGPaint fg2 = nvg.boxGradient(mPos.x + 1, mPos.y + 1 + 1.0f, mSize.x - 2,
                mSize.y - 2, 3, 4, Color(255, 0, 0, 100), Color(255, 0, 0, 50));

        nvg.beginPath;
        nvg.roundedRect(mPos.x + 1, mPos.y + 1 + 1.0f, mSize.x - 2, mSize.y - 2, 3);
        ...
        nvg.beginPath;
        nvg.roundedRect(mPos.x + 0.5f, mPos.y + 0.5f, mSize.x - 1, mSize.y - 1, 2.5f);
        ...
        if (mUnitsImage.valid)
        {
            ...
            NVGPaint imgPaint = nvg.imagePattern(mPos.x + mSize.x - xSpacing - unitWidth,
        ...
        else if (mUnits.length)
        {
            ...
            algn.middle = true;
            nvg.textAlign(algn);
            nvg.text(mPos.x + mSize.x - xSpacing, draw_pos.y, mUnits);
            unitWidth += 2;
        }
        ...
        if (mSpinnable)
        {
            ...
            /* up button */
            {
                ...
                nvg.fillColor((mEnabled && (hover || spinning))
                        ? mTheme.mTextColor : mTheme.mDisabledTextColor);
                auto icon = mTheme.mTextBoxUpIcon;
                NVGTextAlign algn;
                algn.left = true;
                algn.middle = true;
                nvg.textAlign(algn);
                auto iconPos = Vector2f(mPos.x + 4.0f, mPos.y + mSize.y / 2.0f - xSpacing / 2.0f);
            ...
            /* down button */
            {
                ...
                algn.middle = true;
                nvg.textAlign(algn);
                auto iconPos = Vector2f(mPos.x + 4.0f, mPos.y + mSize.y / 2.0f
                        + xSpacing / 2.0f + 1.5f);
                nvg.text(iconPos.x, iconPos.y, [icon]);
            }
        ...
        if (mCommitted)
        {
            nvg.text(draw_pos.x, draw_pos.y, !mValue.length ? mPlaceholder : mValue);
        }
        else
        {
            ...
            auto glyphs = nvg.textGlyphPositions(draw_pos.x, draw_pos.y,
            ...
            glyphs = nvg.textGlyphPositions(draw_pos.x, draw_pos.y, mValueTemp, glyphs_buffer);
            glyphs = squeezeGlyphs(glyphs_buffer[], glyphs);
            ...
            if (mCursorPos > -1)
            {
                ...
                if (mSelectionPos > -1)
                {
                    float selx = cursorIndex2Position(mSelectionPos, textBound[2], glyphs);
                    ...
                    nvg.rect(caretx, draw_pos.y - lineh * 0.5f, selx - caretx, lineh);
                    nvg.fill;
                }
                ...
                if (mEditingValue != "")
                {
                    ...
                    nvg.textBounds(caretx, draw_pos.y, mEditingValue, textBound);

                    nvg.beginPath;
                    nvg.fillColor(Color(255, 255, 255, 255));
                    nvg.rect(caretx, draw_pos.y - lineh * 0.5f, textBound[2] - caretx, lineh);
                    nvg.fill;

                    nvg.fillColor(Color(0, 0, 0, 255));
                    nvg.text(caretx, draw_pos.y, mEditingValue);

                    glyphs = nvg.textGlyphPositions(caretx, draw_pos.y, mEditingValue, glyphs_buffer);
                    glyphs = squeezeGlyphs(glyphs_buffer[], glyphs);

                    caretx = cursorIndex2Position(mEditingCursorPos, textBound[2], glyphs);

                    nvg.beginPath;
                    nvg.moveTo(caretx, draw_pos.y - lineh * 0.5f);
                    nvg.lineTo(caretx, draw_pos.y + lineh * 0.5f);
                    nvg.strokeColor(nvgRGBA(255, 0, 0, 255));
                    nvg.strokeWidth(1.0f);
                    nvg.stroke;
                }
            }
...
protected:
    ...
    bool copySelection()
    {
        if (mSelectionPos > -1)
        {
            ...
            setClipboardText(mValueTemp[begin..end]);
            return true;
        }
    ...
    void pasteFromClipboard()
    {
        if (hasClipboardText())
        {
            auto str = getClipboardText();
            if (str !is null && str.length > 0)
            {
                import std.utf : count;

                auto pos = symbolLengthToBytes(mValueTemp, mCursorPos);
                mValueTemp = mValueTemp[0 .. pos] ~ str ~ mValueTemp[pos .. $];
                mCursorPos += str.count;
    ...
    void updateCursor(NVGContext nvg, float lastx, const(NVGGlyphPosition)[] glyphs)
    {
    ...
    {
        float pos = 0;
        if (glyphs is null || glyphs.length == 0 || index >= glyphs.length)
    ...
    {
        int mCursorId = 0;
        if (glyphs !is null && glyphs.length > 0)
        {
    ...
    {
        if (0 <= pos.x - mPos.x && pos.x - mPos.x < 14.0f)
        { /* on scrolling arrows */
            if (mSize.y >= pos.y - mPos.y && pos.y - mPos.y <= mSize.y / 2.0f)
            { /* top part */
                return SpinArea.Top;
            }
            else if (0.0f <= pos.y - mPos.y && pos.y - mPos.y > mSize.y / 2.0f)
            { /* bottom part */
 ...
 * `uint32_t`, etc.
 */
class ScalarBox(Scalar) : TextBox
{
public:
    this(Widget parent)
    {
    ...
    final void value(Scalar value)
    {
        ...
        super.value = to!string(min(max(value, mMinValue), mMaxValue));
    }

    final Scalar minValue() const { return mMinValue; }
    final void minValue(Scalar value) { mMinValue = value; }
    final Scalar maxValue() const { return mMaxValue; }
    final void maxValue(Scalar value) { mMaxValue = value; }
    final void minMaxValue(Scalar min, Scalar max) { mMinValue = min; mMaxValue = max; }

    final Scalar valueIncrement() const { return mValueIncrement; }
    final void valueIncrement(Scalar value) { mValueIncrement = value; }

    final void callback(void delegate(Scalar) cb)
    {
        super.callback = (string str) {
            import std.conv : to;
            value = to!Scalar(str);
            cb(value);
            return true;
    ...
    override bool mouseButtonEvent(Vector2i p, MouseButton button, bool down, int modifiers)
    {
        if (button == MouseButton.Left)
        {
            ...
            if (mSpinnable && area != SpinArea.None && down) {
    ...
    override bool mouseDragEvent(Vector2i p, Vector2i rel, MouseButton button, int modifiers)
    {
		...
		if (mSpinnable && !focused() && button == MouseButton.Right && mMouseDownPos.x != -1) {
				int valueDelta = cast(int)((p.x - mMouseDownPos.x) / 10.0f);
...
private:
	...
	Scalar mValueIncrement = 1;
	Scalar mMinValue;
    Scalar mMaxValue;
}

class IntBox(Integral) : ScalarBox!Integral
    if (isIntegral!Integral)
{
public:
    this(Widget parent, Integral initValue = cast(Integral)0)
    {
        super(parent);
        format = isSigned!Integral ? "[-]?[0-9]*" : "[0-9]";
        minMaxValue(Integral.min, Integral.max);
        value = initValue;
    }
}

class FloatBox(Float) : ScalarBox!Float
    if (isFloatingPoint!Float)
{
public:
    this(Widget parent, Float initValue = cast(Float)0)
    {
        super(parent);
        format = "[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?";
        valueIncrement = cast(Float)0.1;
        minMaxValue(Float.min_normal, Float.max);
        value = initValue;
    }
}
