...
public:
    ...
    final void side(Side popupSide) { mSide = popupSide; }
    ...
    override bool mouseEnterEvent(Vector2i p, bool enter)
    {
        ...
        auto btn = cast(PopupButton)parent;
        if (btn !is null && btn.PopupOnHover)
        {
            btn.pushed(enter);
        }
    ...
    override void performLayout(NVGContext nvg)
    {
        ...
        final switch (mSide)
        {
            ...
            case Side.Top:
                mAnchorPos[0] -= size[0] / 2;
                mAnchorPos[1] -= size[1] - mAnchorHeight;
                break;

            case Side.Bottom:
                mAnchorPos[0] -= size[0] / 2;
                mAnchorPos[1] += mAnchorHeight;
                break;
        }
    }
    ...
    override void draw(NVGContext nvg)
    {
        ...
        auto halfHeight = mAnchorHeight / 2;
        ...
        if (mSide == Side.Left || mSide == Side.Right)
        {
            ...
            nvg.moveTo(base.x + halfHeight*sign, base.y);
            nvg.lineTo(base.x - 1*sign, base.y - halfHeight);
            nvg.lineTo(base.x - 1*sign, base.y + halfHeight);
        }
        else
        {
            Vector2i base = mPos + Vector2i(width / 2, height);
            int sign = -1;
            ...
            nvg.moveTo(base.x, base.y - halfHeight * sign);
            nvg.lineTo(base.x - halfHeight * sign, base.y - 1);
            nvg.lineTo(base.x + halfHeight * sign, base.y - 1); 
        }
