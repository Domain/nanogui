...
public:
	...
	final Orientation orientation() const
	{
		return mOrientation;
	}
	...
	final void setAlignment(Alignment alignment)
	{
		mAlignment = alignment;
	}
	...
	override Vector2i preferredSize(NVGContext nvg, const Widget widget, const Widget skipped = null) const
	{
		...
		foreach (w; widget.children)
		{
			...
			auto targetSize = Vector2i(fs[0] ? fs[0] : ps[0], fs[1] ? fs[1] : ps[1]);
	...
	override void performLayout(NVGContext nvg, Widget widget) const
	{
		...
		auto containerSize = Vector2i(fs_w[0] ? fs_w[0] : widget.width, fs_w[1]
				? fs_w[1] : widget.height);
...
public:
	 ...
	 */
	this(int margin = 15, int spacing = 6, int groupSpacing = 14, int groupIndent = 20)
	{
		mMargin = margin;
		mSpacing = spacing;
	...
	override void performLayout(NVGContext nvg, Widget widget) const
	{
		int height = mMargin, availableWidth = (widget.fixedWidth()
				? widget.fixedWidth() : widget.width()) - 2 * mMargin;
		...
		foreach (c; widget.children)
		{
			...
			bool indentCur = indent && label is null;
			Vector2i ps = Vector2i(availableWidth - (indentCur ? mGroupIndent
					: 0), c.preferredSize(nvg).y);
...
public:
	...
	override Vector2i preferredSize(NVGContext nvg, const Widget widget, const Widget skipped = null) const
	{
		...
		auto size = Vector2i(2 * mMargin + sum(grid[0][]) + max(cast(int) grid[0].length - 1,
				0) * mSpacing[0], 2 * mMargin + sum(grid[1][]) + max(cast(int) grid[1].length - 1,
				0) * mSpacing[1]);
...
protected:
			...
			ref Array!(int)[2] grid, const Widget skipped) const
	{
		...
		grid[axis1].clear();
		grid[axis1].length = dim[axis1];
		grid[axis1][] = 0;
		grid[axis2].clear();
		grid[axis2].length = dim[axis2];
		grid[axis2][] = 0;
...
public:
    ...
    */
	struct Anchor
	{
		...
		Alignment[2] _align; ///< The ``(x, y)`` Alignment.

		/// Create an Anchor at position ``(x, y)`` with specified Alignment.
		this(int x, int y, Alignment horiz = Alignment.Fill, Alignment vert = Alignment.Fill)
		{
			pos[0] = cast(ubyte) x;
			pos[1] = cast(ubyte) y;
			size[0] = size[1] = 1;
		...
		this(int x, int y, int w, int h, Alignment horiz = Alignment.Fill,
	...
	this(Array!int cols = Array!int(), Array!int rows = Array!int(), int margin = 0)
	{
		mCols = cols;
		mRows = rows;
		mMargin = margin;
	...
	final int colCount() const
	{
		return cast(int) mCols.length;
	}

	/// Return the number of rows
	final int rowCount() const
	{
		return cast(int) mRows.length;
	}

	/// Append a row of the given size (and stretch factor)
	final void appendRow(int size, float stretch = 0.0f)
	{
		mRows ~= size;
		mRowStretch ~= stretch;
	};
	...
	final void appendCol(int size, float stretch = 0.0f)
	{
		mCols ~= size;
		mColStretch ~= stretch;
	};
	...
	final void setRowStretch(int index, float stretch)
	{
		mRowStretch[index] = stretch;
	}

	/// Set the stretch factor of a given column
	final void setColStretch(int index, float stretch)
	{
		mColStretch[index] = stretch;
	}

	/// Specify the anchor data structure for a given widget
	final void setAnchor(Widget widget, Anchor anchor)
	{
		mAnchor[widget] = anchor;
	}

	/// Retrieve the anchor data structure for a given widget
	final Anchor anchor(Widget widget) const
	{
		auto it = widget in mAnchor;
		if (it is null)
			throw new Error("Widget was not registered with the grid layout!");
		return *it;
	}
	...
	override Vector2i preferredSize(NVGContext nvg, const Widget widget, const Widget skipped = null) const
	{
		...
		Vector2i size = Vector2i(sum(grid[0][]), sum(grid[1][]));

		Vector2i extra = Vector2i(2 * mMargin, 2 * mMargin);
		const Window window = cast(const Window)(widget);
		if (window !is null && window.title().length > 0)
			extra[1] += widget.theme().mWindowHeaderHeight - mMargin/2;
	...
	override void performLayout(NVGContext nvg, Widget widget) const
	{
		...
		for (int axis=0; axis<2; ++axis) {
			...
			foreach (Widget w; widget.children()) {
				if (!w.visible())
					continue;
				...
				int ps = w.preferredSize(nvg)[axis], fs = w.fixedSize()[axis];
				int targetSize = fs ? fs : ps;

				final switch (anchor._align[axis]) {
					case Alignment.Minimum:
						break;
					case Alignment.Middle:
...
protected:
	/// Computes the layout
	void computeLayout(NVGContext nvg, const Widget widget, ref Array!(int)[2] grid, const Widget skipped) const
	{
		Vector2i fs_w = widget.fixedSize;
		Vector2i containerSize = Vector2i(
		...
		for (int axis=0; axis<2; ++axis) {
			auto sizes = axis == 0 ? &mCols : &mRows;
			const auto stretch = axis == 0 ? &mColStretch : &mRowStretch;
			//grid[axis] = sizes[0..sizes.length];

			for (int phase = 0; phase < 2; ++phase) {
				foreach (key, ref value; mAnchor) {
					const Widget w = key;
					if (!w.visible())
						continue;
					const auto anchor = &value;
					if ((anchor.size[axis] == 1) != (phase == 0))
						continue;
					int ps = w.preferredSize(nvg)[axis], fs = w.fixedSize()[axis];
					int targetSize = fs ? fs : ps;

					if (anchor.pos[axis] + anchor.size[axis] > cast(int) grid[axis].length)
						throw new Error(
					...
					for (int i = anchor.pos[axis];
						...
						if ((*sizes)[i] == 0 && anchor.size[axis] == 1)
							grid[axis][i] = max(grid[axis][i], targetSize);
						currentSize += grid[axis][i];
					...
					for (int i = anchor.pos[axis];
						i < anchor.pos[axis] + anchor.size[axis]; ++i) {
						grid[axis][i] += cast(int) round(amt * (*stretch)[i]);
					}
				}
			}

			int currentSize = sum(grid[axis][]);
			...
			for (size_t i = 0; i<grid[axis].length; ++i)
				grid[axis][i] += cast(int) round(amt * (*stretch)[i]);
		}
	}
